...
const DialPhoneAutoComplete = (props) => {
  ...
  const dial_code = useSelector((state) =>
    props.smallForm
      ? state.form.dialing_code
  ...
  const getUserCountry = async () => {
    ...
    if (dialCodeMap[dial_code]?.name) {
      console.log("INSIDE THE IF ", dialCodeMap[dial_code]?.name);
  ...
  const fetchCountry = () => {
    let duplicatedArray = [...countries];
    let sortDialingCode = duplicatedArray?.sort(function (a, b) {
  ...
  const handleChange = (e, inputValue) => {
    console.log("blabla", e.target.id, e.target.name, inputValue);
    setCountryDialCode(inputValue);
    props.handleChange(e, "", props.index);
  };
  console.log("INPUT CODE", countryDialCodeInput, countryDialCode, userCountry);
  return (
    <StyledAutoComplete
      id="contact_phone"
      name="contact_phone"
