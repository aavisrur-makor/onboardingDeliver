...
const FinaleBox = () => {
  ...
  const AcceptAndSendFinish = useSelector((state) => state.auth.AcceptAndSendFinish)
  const remainingFields = useSelector((state) => state.validation.remainingFields)
  ...
  return (
    <Modal open={AcceptAndSendFinish} className={classes.container}>
      <Grid justifyContent='center' alignItems='center' direction='column' container item className={classes.subContainer}>
        {remainingFields.length ? (
          ...
          <Typography style={{ font: 'normal normal normal 24px/28px Work Sans' }}>Your application has been successfully sent</Typography>
        )}
