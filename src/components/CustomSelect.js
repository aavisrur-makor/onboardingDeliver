...
const CustomSelect = (props) => {
  ...
  return (
    <FormControl error={!requiredField && isFormSubmitted && props.required} fullWidth={props.fullWidth ?? true} variant='outlined'>
      <InputLabel required={props.required} id='demo-simple-select-outlined-label'>
        {props.label}
      </InputLabel>
      {(stateDataMap && stateDataMap[value]) || props.selectData || (props.stateDataMap && props.stateData) ? (
        <Select
          ...
          value={props.selectData ? value : stateDataMap[value] ? stateDataMap[value] : ''}
          onChange={(e, child) => {
        ...
        >
          {stateData
            ...
            : props.selectData?.map((option, index) => {
                return (
                  <MenuItem style={{ textTransform: 'capitalize' }} id={option} value={option}>
                    {props.id === 'trading_volume' ? (index === props.selectData.length - 1 ? `${numeral(option).format('$0,0')}+` : numeral(option).format('$0,0')) : option}
                  </MenuItem>
                )
              })}
      ...
      {!requiredField && isFormSubmitted && props.required && <FormHelperText>This field is required!</FormHelperText>}
    </FormControl>
  )
}
